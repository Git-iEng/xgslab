...
function showToast(msg, type='error', timeoutMs=4000){
  ...
  el.className = 'xgslab-toast ' + (type==='ok' ? 'xgslab-toast--ok' : 'xgslab-toast--error');
  el.innerHTML = `<span aria-hidden="true">${type==='ok'?'✔️':'⚠️'}</span>
                  <div>${msg}</div>
                  <button class="xgslab-toast__close" aria-label="Close">×</button>`;
  root.appendChild(el);
  const remove = () => el.remove();
  el.querySelector('.xgslab-toast__close').addEventListener('click', remove);
