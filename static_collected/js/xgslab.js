...
(function () {
  const cards = document.querySelectorAll('.card-xgslab-cards');
  const io = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      if (e.isIntersecting) {
        e.target.classList.add('in-view-xgslab-cards');
      } else {
        e.target.classList.remove('in-view-xgslab-cards'); // replay when scrolling back
      }
    });
  }, { threshold: 0.18 });
...
(function () {
  const rows = document.querySelectorAll('.row-xgslab-description');
  const io = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      e.target.classList.toggle('in-view-xgslab-description', e.isIntersecting);
    });
  }, { threshold: 0.15 });
...
(function () {
  const rail = document.getElementById('rail-xgslab-testimonials');
  const prev = document.querySelector('.xgslab-testimonials .prev') || document.querySelector('.prev');
  const next = document.querySelector('.xgslab-testimonials .next') || document.querySelector('.next');
  const bar = document.querySelector('.bar-xgslab-testimonials');

  function slide(dir = 1) {
    const card = rail.querySelector('.card-xgslab-testimonials');
...
(function () {
  const rail = document.getElementById('rail-xgslab-testimonials');
  const prev = document.querySelector('.nav-xgslab-testimonials.prev');
  const next = document.querySelector('.nav-xgslab-testimonials.next');
  const bar = document.querySelector('.bar-xgslab-testimonials');
  if (!rail || !prev || !next || !bar) return;

  // step size = card width + column gap
  const gap = () => parseInt(getComputedStyle(rail).columnGap || getComputedStyle(rail).gap || 24, 10);
  const step = () => (rail.querySelector('.card-xgslab-testimonials')?.offsetWidth || 520) + gap();
...
(function () {
  const grid = document.getElementById('grid-xgslab-card-with-animation');
  if (!grid) return;
  const cards = grid.querySelectorAll('.card-xgslab-card-with-animation');

  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('show-xgslab-card-with-animation');
      } else {
        // remove so it replays when scrolling back (LIFO feel)
        e.target.classList.remove('show-xgslab-card-with-animation');
      }
    });
  }, { threshold: 0.18 });
